  ...
  public static void writeXML(User user, String filePath) {
    try {
      ...
      for (OurEvent ourEvent : user.getSchedule().getEvents()) {
        Element eventElement = doc.createElement("ourEvent");
        rootElement.appendChild(eventElement);
        Element name = doc.createElement("name");
        name.appendChild(doc.createTextNode(ourEvent.getName()));
        eventElement.appendChild(name);
        Element time = doc.createElement("time");
        eventElement.appendChild(time);
        createChildElement(doc, time, "start-day", ourEvent.getStartDay());
        createChildElement(doc, time, "start", ourEvent.getStartTime());
        createChildElement(doc, time, "end-day", ourEvent.getEndDay());
        createChildElement(doc, time, "end", ourEvent.getEndTime());
        Element location = doc.createElement("location");
        eventElement.appendChild(location);
        createChildElement(doc, location, "online", String.valueOf(ourEvent.getOnlineStatus()));
        createChildElement(doc, location, "place", ourEvent.getLocation());
