...
public class CalendarUI extends JFrame implements PlannerGrid {
  ...
  private HashMap<Integer, OurEvent> intEvent; // Declare this as a class-level attribute.
  ...
  private void setupListeners() {
    table.addMouseListener(new MouseAdapter() {
      public void mouseClicked(MouseEvent e) {
        ...
        int currTime = column * 1440 + row;
        OurEvent clickedOurEvent = intEvent.get(currTime);
        if (clickedOurEvent != null) {
          EventFrame eventFrame = new EventFrame(clickedOurEvent, model, features, null);
          eventFrame.render();
        }
      }
    });
  ...
  private HashMap<Integer, OurEvent> printEventsForCurrentUser() {
    HashMap<Integer, OurEvent> timeToEvent = new HashMap<>();

    if (currentUser != null && currentUser.getSchedule() != null) {
      List<OurEvent> ourEvents = currentUser.getSchedule()
          .getEvents(); // Get the ourEvents list of the current user
      for (OurEvent e : ourEvents) {
   ...
   */
  private void fillCalendarWithEvents(HashMap<Integer, OurEvent> events) {
    DefaultTableModel model = (DefaultTableModel) table.getModel();
    model.setRowCount(0); // Clear existing content
    model.setRowCount(1440); // Reset rows for a new day

    for (Map.Entry<Integer, OurEvent> entry : events.entrySet()) {
